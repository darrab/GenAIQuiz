...
st.set_page_config(page_title="Deep Learning Quiz", page_icon="🎓", layout="wide")
...
with st.sidebar:
    st.title("🎓 DL Quiz Master")
    st.markdown("---")
    
    lecture_options = ["All", "Lecture 01", "Lecture 02", "Lecture 03", "Lecture 04", "Lecture 05", "Lecture 06"]
    ...
    if st.button("🚀 Start New Quiz"):
        ...
        # Filter questions
        if selected_lecture == "All":
            st.session_state.quiz_questions = QUESTIONS.copy()
        else:
            lec_num = int(selected_lecture.split()[1])
    ...
    if st.session_state.quiz_started:
        st.success(f"Quiz Questions: {len(st.session_state.quiz_questions)}")

# Main content
st.title("🎓 Deep Learning & Gen AI Quiz")
st.markdown("### Test your knowledge across 6 lectures!")
...
if not st.session_state.quiz_started:
    ...
    **Instructions:**
    1. Select a lecture from the sidebar (or 'All' for full quiz)
    ...
    **Quiz Features:**
    - ✅ 300 Total Questions (50 per lecture)
    - ✅ Randomized answer positions
    - ✅ Instant feedback with explanations
    - ✅ Score tracking
    - ✅ Lecture-specific or full quiz mode
    """)
    ...
    if st.session_state.show_result:
        ...
        incorrect = [a for a in st.session_state.answers if not a['correct']]
        if incorrect:
            with st.expander(f"Review {len(incorrect)} Incorrect Answers"):
        ...
        st.markdown(f"### Question {st.session_state.current_question + 1}/{total_questions}")
        ...
        user_answer = st.radio("Select your answer:", list(options.keys()), key=f"q_{st.session_state.current_question}")
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Submit Answer", type="primary"):
                correct = user_answer == q['ans']
                if correct:
...
# Footer
st.markdown("---")
st.markdown("Built with ❤️ using Streamlit | Deep Learning & Gen AI Course")
